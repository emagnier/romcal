//! Martyrology entry merging utilities.
//!
//! This module provides functions for merging martyrology entries across locales.

use std::collections::BTreeMap;

use super::locale::build_merge_hierarchy;
use crate::romcal::Romcal;
use crate::types::martyrology::{MartyrologyEntry, MartyrologyEntryDef, MartyrologyEntryId};

/// Merge martyrology entries from all locales in the hierarchy.
///
...
pub(crate) fn merge_locale_resources(
    romcal: &Romcal,
) -> BTreeMap<MartyrologyEntryId, MartyrologyEntry> {
    let mut merged: BTreeMap<MartyrologyEntryId, MartyrologyEntry> = BTreeMap::new();
    ...
    for locale in hierarchy {
        if let Some(resources) = romcal.get_resources(&locale)
            && let Some(martyrology) = &resources.martyrology
        {
            for (id, definition) in martyrology {
                if let Some(existing) = merged.get_mut(id) {
                    // Merge: more specific locale properties override base
                    merge_entry_from_definition(existing, definition);
                } else {
                    // New entry from this locale
                    merged.insert(
                        id.clone(),
                        MartyrologyEntry::new(id.clone(), definition.clone()),
                    );
                }
            }
        }
...
/// Merges properties from a MartyrologyEntryDef into an existing MartyrologyEntry.
/// Source properties override target properties when defined.
pub(crate) fn merge_entry_from_definition(
    target: &mut MartyrologyEntry,
    source: &MartyrologyEntryDef,
) {
