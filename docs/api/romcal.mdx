---
title: Romcal Class
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

The main class for generating liturgical calendars.

## Constructor

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
const romcal = new Romcal(options?: RomcalOptions);
```

  </TabItem>
  <TabItem label="Python">

```python
romcal = Romcal(**options)
```

  </TabItem>
  <TabItem label="Rust">

```rust
let romcal = Romcal::new();
// or with builder pattern
let romcal = Romcal::new()
    .calendar("france")
    .locale("fr");
```

  </TabItem>
</Tabs>

## Methods

### generateCalendar

Generate a liturgical calendar for a given year.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
generateCalendar(year: number): LiturgicalDay[]
```

  </TabItem>
  <TabItem label="Python">

```python
def generate_calendar(self, year: int) -> list[LiturgicalDay]
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn generate_calendar(&self, year: i32) -> Vec<LiturgicalDay>
```

  </TabItem>
</Tabs>

### generateMasses

Generate a mass-centric calendar for a given year.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
generateMasses(year: number): Mass[]
```

  </TabItem>
  <TabItem label="Python">

```python
def generate_masses(self, year: int) -> list[Mass]
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn generate_masses(&self, year: i32) -> Vec<Mass>
```

  </TabItem>
</Tabs>

### getDate

Get a specific liturgical date.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
getDate(dateId: string, year: number): string
```

  </TabItem>
  <TabItem label="Python">

```python
def get_date(self, date_id: str, year: int) -> str
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn get_date(&self, date_id: &str, year: i32) -> String
```

  </TabItem>
</Tabs>

### getMartyrologyEntry

Lookup a martyrology entry by its exact ID.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
getMartyrologyEntry(entryId: string): MartyrologyEntry | null
```

  </TabItem>
  <TabItem label="Python">

```python
def get_martyrology_entry(self, entry_id: str) -> MartyrologyEntry | None
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn get_martyrology_entry(&self, entry_id: &str) -> Option<MartyrologyEntry>
```

  </TabItem>
</Tabs>

### searchMartyrologyEntries

Search for martyrology entries with fuzzy matching.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
searchMartyrologyEntries(query: MartyrologyQuery): MartyrologySearchResult[]
```

  </TabItem>
  <TabItem label="Python">

```python
def search_martyrology(self, query: MartyrologyQuery) -> list[MartyrologySearchResult]
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn search_martyrology(&self, query: MartyrologyQuery) -> Vec<MartyrologySearchResult>
```

  </TabItem>
</Tabs>

## Static Methods

### getAvailableCalendars

Get a list of all available calendars.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
static getAvailableCalendars(): string[]
```

  </TabItem>
  <TabItem label="Python">

```python
@staticmethod
def get_available_calendars() -> list[str]
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn available_calendars() -> Vec<String>
```

  </TabItem>
</Tabs>

### getAvailableLocales

Get a list of all available locales.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
static getAvailableLocales(): string[]
```

  </TabItem>
  <TabItem label="Python">

```python
@staticmethod
def get_available_locales() -> list[str]
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn available_locales() -> Vec<String>
```

  </TabItem>
</Tabs>

### mergeCalendarDefinitions

Merge multiple calendar definition objects into a single combined definition. Useful for combining custom definitions with built-in calendars.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
static mergeCalendarDefinitions(...definitions: CalendarDefinition[]): CalendarDefinition
```

  </TabItem>
  <TabItem label="Python">

```python
@staticmethod
def merge_calendar_definitions(*definitions: CalendarDefinition) -> CalendarDefinition
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn merge_calendar_definitions(definitions: &[CalendarDefinition]) -> CalendarDefinition
```

  </TabItem>
</Tabs>

**Example:**

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
import { Romcal } from 'romcal';

const customDefinitions = {
  /* ... */
};
const merged = Romcal.mergeCalendarDefinitions(builtInDefs, customDefinitions);
```

  </TabItem>
  <TabItem label="Python">

```python
from romcal import Romcal

custom_definitions = { ... }
merged = Romcal.merge_calendar_definitions(built_in_defs, custom_definitions)
```

  </TabItem>
  <TabItem label="Rust">

```rust
use romcal::Romcal;

let merged = Romcal::merge_calendar_definitions(&[built_in_defs, custom_definitions]);
```

  </TabItem>
</Tabs>

### mergeResourceFiles

Merge multiple resource files (martyrology translations) into a single combined resource object. Useful for adding custom translations or overriding existing ones.

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
static mergeResourceFiles(...resources: ResourceFile[]): ResourceFile
```

  </TabItem>
  <TabItem label="Python">

```python
@staticmethod
def merge_resource_files(*resources: ResourceFile) -> ResourceFile
```

  </TabItem>
  <TabItem label="Rust">

```rust
fn merge_resource_files(resources: &[ResourceFile]) -> ResourceFile
```

  </TabItem>
</Tabs>

**Example:**

<Tabs syncKey="lang">
  <TabItem label="TypeScript">

```typescript
import { Romcal } from 'romcal';

const customResources = {
  locale: 'en',
  martyrology: {
    my_custom_saint: {
      fullname: 'Saint Custom Name',
      canonization_level: 'SAINT',
    },
  },
};
const merged = Romcal.mergeResourceFiles(builtInResources, customResources);
```

  </TabItem>
  <TabItem label="Python">

```python
from romcal import Romcal

custom_resources = {
    "locale": "en",
    "martyrology": {
        "my_custom_saint": {
            "fullname": "Saint Custom Name",
            "canonization_level": "SAINT",
        },
    },
}
merged = Romcal.merge_resource_files(built_in_resources, custom_resources)
```

  </TabItem>
  <TabItem label="Rust">

```rust
use romcal::Romcal;
use serde_json::json;

let custom_resources = json!({
    "locale": "en",
    "martyrology": {
        "my_custom_saint": {
            "fullname": "Saint Custom Name",
            "canonization_level": "SAINT"
        }
    }
});

let merged = Romcal::merge_resource_files(&[built_in_resources, custom_resources]);
```

  </TabItem>
</Tabs>
