 ...
 */
export type DateDefExceptions = DateDefException | DateDefException[];
...
export interface DayDefinition {
  /**
   * In addition to this liturgical day, allow similar items that have the same rank, and the same or lower precedence, so the current liturgical day will not overwrite another defined item.
   */
  allow_similar_rank_items?: boolean | null;
  /**
   * The liturgical color(s) of the liturgical day. @deprecated
   */
  colors?: ColorsDef | null;
  /**
   * The **Common** refers to a set of prayers, readings, and chants used for celebrating saints or feasts that belong to a specific category, such as martyrs, virgins, pastors, or the Blessed Virgin Mary.
   */
  ...
  /**
   * If this liturgical day must be removed from this calendar and from all those it inherits (the parent calendars), on the final calendar generated by romcal.
   */
  drop?: boolean | null;
  id: string;
  /**
   * Holy days of obligation are days on which the faithful are expected to attend Mass, and engage in rest from work and recreation.
   */
  is_holy_day_of_obligation?: boolean | null;
  /**
   * Specify is this LiturgicalDay is optional within a specific liturgical calendar.
   *
   * UNLY #14: Memorials are either obligatory or optional; their observance is integrated into the celebration of the occurring weekday in accordance with the norms set forth in the General Instruction of the Roman Missal and of the Liturgy of the Hours
   *
   * Note: also used for the dedication of consecrated churches, which is an optional solemnity that should not overwrite the default weekday.
   */
...
/**
 * The liturgical day date exception Represents a condition and the date to set when that condition is met
 */
...
export interface ParticularConfig {
  /**
   * Configuration options specific to this calendar. These settings can override or extend the default Romcal configuration or any parent calendar configuration.
   */
